...
exports.Home = (req, res) => {
  ...
  res.status(200).json({
    ...
    success: true,
    message: "Welcome to Pabitra Memorial School"
...
exports.loginUser = catchAsyncError(async (req, res, next) => {
  ...
  const message = `You have successfully logged in to Pabitra Memorial School Website`;
  try {
    await sendEmail({
      email: user.email,
      subject: "Login to Pabitra Memorial School Website",
      message,
    });
...
exports.admissionEnquiry = catchAsyncError(async (req, res, next) => {
  ...
  const user = await AdmissionQuery.findOne({
    email
  });
...
exports.getAllVisitSchedule = catchAsyncError(async (req, res, next) => {

  const visit = await ScheduleVisit.find().sort({
    createdAt: -1
  })
...
exports.deleteScheduleVisit=catchAsyncError(async(req,res,next)=>{

  const {id}=req.params;


 const visit=await ScheduleVisit.findById(id)

 if(!visit){
  return next(new ErrorHandler("visit schedule not found",404));
 }

 await ScheduleVisit.deleteOne({_id:id})

  res.status(200).json({
      success:true,
      message:"Visit Schedule data deleted successfully"
  })
